...
function EnterGame.init()
  ...
  if not ALLOW_CUSTOM_SERVERS and Servers then
    for name, server in pairs(Servers) do
      serverSelector:setCurrentOption(name, false)
      G.server = name
      if type(server) == 'string' then
        G.host = server
        local params = server:split(':')
        if #params >= 3 then
          G.clientVersion = tonumber(params[3]) or APP_VERSION
        end
      end
      g_settings.set('server', name)
      g_settings.set('host', G.host or server)
      if G.clientVersion then
        g_settings.set('client-version', G.clientVersion)
        clientVersionSelector:setOption(tostring(G.clientVersion))
      end
      break
    end
  end
...
function EnterGame.doLogin(account, password, token, host)
  ...
  if not ALLOW_CUSTOM_SERVERS and Servers and Servers[G.server] then
    G.host = Servers[G.server]
    local params = G.host:split(':')
    if #params >= 3 then
      G.clientVersion = tonumber(params[3]) or APP_VERSION
    end
  else
